---
## get-shit-done / AGENTS

### 來源
- repo：get-shit-done
- 路徑：docs/pt-BR/AGENTS.md
- 檔案類型：AGENTS.md
- card_kind：translation
- language：pt-BR
- canonical_group：get-shit-done::docs/agents.md

### 一句話定位
Este documento descreve os papéis dos agentes especializados no ecossistema GSD.

### 核心人格特質
框架導向, 第一性原理, 推理導向, 流程紀律, 完成導向, 規範導向, 元認知

### 核心思考框架
原文過短，無法提取

### 核心行為規則
必須做
- 原文過短，無法提取

禁止做
- 原文過短，無法提取

### 提問方式
無明確提問模板

### 審查維度
非審查型

### 輸出格式要求
無特定格式要求

### 適用場景
- 適合在需要「Este documento descreve os papéis dos agentes especializados no ecossistema GSD.」的工作階段使用。

### 原文精華摘錄
> Este documento descreve os papéis dos agentes especializados no ecossistema GSD
> Para a listagem completa com regras detalhadas, consulte [AGENTS.md em inglês](../AGENTS.md)
> Visão geral
> O GSD usa um **orquestrador leve** para coordenar subagentes especializados por etapa:
> - pesquisa
> - planejamento
> - execução
> Famílias de agentes
> - **Project/Phase researchers**: investigam stack, arquitetura, padrões e riscos
> - **Research synthesizer**: consolida descobertas em artefatos utilizáveis

### 和其他 skill 的潛在關聯
- README（pm-skills） - 相似 - 共享領域：meta, shipping；共享分類：思考框架型、流程型、元技能型；共同關鍵詞：com
- algorithmic-art（anthropics-skills） - 相似 - 共享領域：meta, shipping；共享分類：思考框架型、流程型、元技能型
- claude-api（anthropics-skills） - 相似 - 共享領域：meta, shipping；共享分類：思考框架型、流程型、元技能型
- doc-coauthoring（anthropics-skills） - 相似 - 共享領域：meta, shipping；共享分類：思考框架型、流程型、元技能型

### 分類標記
- [x] 思考框架型
- [ ] 審查型
- [ ] 工具程序型
- [ ] 人格型
- [x] 流程型
- [x] 元技能型
---
