...
本文件的編譯流程
  → output/
      ├── raw-cards/（碎片卡片）
      ├── 00_總索引.md（全卡片索引）
      ├── 01_分類彙整.md（六大分類歸類 ← 裝備熔爐挑卡依據）
      ├── 02_工具清單.md
      └── compile_log.json
```

**01_分類彙整.md 是 compiler 自動產出的分類數據。裝備熔爐鍛造時根據這份分類挑卡，不是自己分類。**
...
|------|---------|---------|---------|--------|
| `compile` | 「執行編譯」「重新編譯」 | 本文件全部 | output/ | 裝備定義、職業定義 |

**compile 模式不得修改裝備或職業。** 要改裝備必須到30_裝備熔爐.md，切 persona_edit 模式。
...
| 裝備重鍛 | 使用者明確說「重鍛 #10X」 | 版本時間、指定裝備、素材快照 |

**普通編譯不得動裝備定義。** 要改裝備必須到30_裝備熔爐.md 操作。
...
| 主索引卡片數變動超過 ±15% | 先人工複核 |
| agent 仍為 0 | 禁止重鍛 #203 |
| empty_framework 或 empty_behavior 比上一版更差 | 禁止直接更新裝備引用 |
| 新 repo 替換舊 repo 但無法覆蓋原核心素材 | 禁止直接替換裝備引用 |
