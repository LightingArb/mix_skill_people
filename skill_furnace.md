...
- `#101`：只複製，不進入編譯流程
- `#102`、`#103`、`#104`、`#201`、`#202`、`#204`、`#212`：正常編譯
- `#211`、`#213`：用 skill-pack 簡化格式編譯
- `#203`：跳過（draft-disabled）

正式順序如下：

`#101（複製）→ #102 → #103 → #104 → #201 → #202 → #204 → #211 → #212 → #213`

每個人格在處理前都會先出挑卡報告等你確認，確認後才產出到 `data/preset-personas/`。

**⚠️ 不要把兩步合成一句。**
第一步有停止條件，如果 compile 出問題但直接跑第二步，會用壞的原料產出壞的人格。
中間那個確認點很重要。
...
- 版本：2.2.0
- 最後編譯時間：2026-03-30T19:46:07
- 最後確認人：Remy（2026-03-30 確認 compile OK + 批次人格編譯）
- 人格內容狀態：見 §5-B 編譯狀態追蹤表
...
| `persona_edit` | 「寫 #102」「重編 #103」「編譯所有人格」 | §5-B | §5-B 編譯狀態追蹤表、§0 | data/preset-personas/ | §2-§4 |
...
| 與「編譯狀態追蹤表」中 status=`已編譯` 的人格核心層重疊 > 2 張 | 停止，報告重疊清單，等人工決定合併或調整 |
...
- 將對應行的 status 更新為 `已編譯`
- 將 data/ 檔案欄更新為 `✓`
- **此步驟必須在寫入 data/ 成功後立刻執行，不得延遲到報告之後**
...
**跳過規則：** 批次編譯時，對每個人格只看「編譯狀態追蹤表」：
- status 為 `已編譯` → 跳過，輸出「#XXX 已編譯，跳過」
- status 為 `待編譯` → 正常編譯
- status 為 `模板（只複製）` → 自動複製 §6-1 到 data/，不進入編譯流程
- status 為 `draft-disabled` → 跳過
- 使用者明確說「重編 #XXX」時 → 無視 status，強制重新編譯並覆蓋

使用者說「編譯所有人格」時，按以下順序逐個處理：
```
#101（複製）→ #102 → #103 → #104 → #201 → #202 → #204 → #211 → #212 → #213
```

注意：
- **#101 不走編譯流程**——它是模板，只複製。只在使用者明確說「重編 #101」時才重新編譯。
...
> **唯一狀態來源。** AI 判斷人格是否需要編譯，只看本表。§6/§7 的 metadata 不再有 status 欄位。
> 你要重編某個人格 → 把本表對應行的 status 改成 `待編譯`。
> AI 每完成一個人格編譯 → 立刻更新本表對應行。
